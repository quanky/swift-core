    ...
    open override func setValue(_ value: Any?, forKey key: String) {
        if let newValue = value {
            ...
            if let prop = property {
                        ...
                        if (newValue is NSArray) {
                            ...
                            if isArrayKind {
                                if let arrayClass = self.classForArrayWithName(name: newKey) {
                                    print("You should declare class for array element with key \(newKey)")
                            ...
                            if (newValue is String &&  isDateKind) {
                                print("Found date value \(newValue) for key \(newKey)")
                ...
                print("Couldnt find property associated with key \(newKey)")
                return
            }
            super.setValue(value, forKey: key)
        } else {
            print("Will ignore nil value for key \(key)")
        }
    }
